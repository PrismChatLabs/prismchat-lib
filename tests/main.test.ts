...
test('Prism Object Encrypt & Decrypt', async () => {
	...
	let encryptedPrismObj = Alice.encryptPrismObject(
		data,
		aliceSharedSessionKeys.sendKey,
		Bob.IdentityKeys.public
	);

	let decryptedPrismObj = Bob.decryptPrismObject(
		encryptedPrismObj,
		bobSharedSessionKeys.receiveKey
	);

	expect(decryptedPrismObj.data).toStrictEqual(data);
	expect(decryptedPrismObj.from).toStrictEqual(Alice.IdentityKeys.public);
});
